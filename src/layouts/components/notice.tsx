  ...
  return (
    <>
      ...
      <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
        <SheetContent
          side="right"
          className="sm:max-w-md p-0 [&>button]:hidden flex flex-col"
          style={style}
        >
...
function NoticeTab() {
  ...
  const renderNotification = (notification: any) => {
    ...
    return (
      <div
        key={notification.id}
        className="flex items-start space-x-3 py-4 border-b border-border last:border-b-0"
      >
          ...
          {notification.files && (
            <div className="mt-3 space-y-2">
              {notification.files.map((file: any) => (
                <div
                  key={file.name}
                  className="p-3 bg-bg-neutral rounded-lg flex items-center space-x-3"
                >
